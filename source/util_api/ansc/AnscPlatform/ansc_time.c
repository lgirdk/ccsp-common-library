...
static   LARGE_INTEGER              gCounterUnit        = {0};
static   LARGE_INTEGER              gStartCounter       = {0};
...
{
    if ( !QueryPerformanceFrequency(&gCounterUnit) )
    {
        AnscTraceWarning(("UserProfilingStartPcc -- no high resulotion performance counter installed!"));
    }
...
{
    LARGE_INTEGER                   stopCounter         = {0};

    QueryPerformanceCounter(&stopCounter);

    stopCounter.QuadPart -= gStartCounter.QuadPart;
    stopCounter.QuadPart  = stopCounter.QuadPart * 1000000 / gCounterUnit.QuadPart; /* in micro seconds */

    return  (ULONG)stopCounter.QuadPart;
}
